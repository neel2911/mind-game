      ...
      <Tooltip>
        ...
        >
          <div className="space-y-3">
            <h4 className="font-semibold text-purple-100 text-sm">
            ...
            <div className="flex items-center justify-center gap-4 rounded-md bg-black/40 p-3 text-xs font-mono border border-white/5">
              <div className="text-center text-yellow-400">
                ...
                <span className="block">Total Attempts</span>
              </div>
              <div className="text-indigo-400 font-bold text-sm">× 100</div>
            </div>

            <p className="text-xs text-slate-400 leading-relaxed">
              We compare the perfect game (minimum moves) against your actual
              moves to determine your accuracy percentage.
            </p>
          </div>
        </TooltipContent>
      </Tooltip>
...
export const GameStatsHeader = () => {
  const { settings, moves } = useSettings();
  const { playerName, difficulty, highScore } = settings;
    ...
    <div className="flex items-center h-16 gap-3 bg-slate-900/50 backdrop-blur-md p-2 rounded-xl border border-slate-800">
      ...
      <div className="flex flex-col items-center px-4 py-1">
        ...
        <span className="font-mono text-amber-400 font-semibold flex items-center justify-between">
          {calculateScore(Number(difficulty), Number(moves))}%
        </span>
      </div>
    </div>
  );
